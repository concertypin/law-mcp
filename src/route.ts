...
import { fetchLawSearch, fetchLawService, normalizeParagraphs } from "./law-api";
...
app.registerTool(
    ...
    async (i) => {
        ...
        const authKey = (globalThis as any).AUTH_KEY as string;
        const searchData = await fetchLawSearch(i.query, authKey);
        
        const total = searchData.LawSearch.totalCnt;
        ...
        const results = await Promise.all(laws.map(async (law) => {
            ...
            try {
                ...
                const articles = lawData.Law.조문.조문단위;
                
                const purposeArticle = articles.find(
                    (a) => a.조문번호 === "1" && a.조문여부 === "조문"
                );

                if (purposeArticle && purposeArticle.조문제목 === "목적") {
                    description = purposeArticle.조문내용 || "";
                    const paragraphs = normalizeParagraphs(purposeArticle.항);
...
app.registerTool(
    ...
    async (i) => {
        ...
        const law_name = lawData.Law.기본정보?.법령명한글 || "법령명 알 수 없음";
        
        const articlesData = lawData.Law.조문.조문단위.filter((a) => a.조문여부 === "조문");
        const articles = articlesData.map((a) => {
            const article_no = a.조문가지번호 ? `${a.조문번호}의${a.조문가지번호}` : a.조문번호;
...
app.registerTool(
    ...
    async (i) => {
        ...
        const articlesData = lawData.Law.조문.조문단위.filter((a) => a.조문여부 === "조문");
        const article = articlesData.find((a) => a.조문번호 === no && a.조문가지번호 === sub);

        if (!article) {
            return {
                content: [{ type: "text", text: `에러: 조문 ${i.article_no}를 찾을 수 없습니다.` }],
                isError: true,
            };
        }
        ...
        if (i.paragraph_no) {
            const para = paragraphs.find((p) => p.항번호 === i.paragraph_no);
            if (!para) {
                return {
                    content: [{ type: "text", text: `에러: 조문 ${i.article_no}의 항 ${i.paragraph_no}를 찾을 수 없습니다.` }],
                    isError: true,
                };
            }
            text = para.항내용;
